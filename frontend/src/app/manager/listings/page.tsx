...
import { formatDate, formatPrice, generateToastId } from '@/src/lib/listing-formatter';
...
const ListingsManagement = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(true); // Will implement later
  ...
  const fetchPendingListings = useCallback(async (): Promise<void> => {
    try {
      ...
      addToast('Failed to fetch pending listings. Please try again.', 'error');
    }
  }, [addToast]);
  ...
  const fetchActiveListings = useCallback(async (): Promise<void> => {
    try {
      ...
      addToast('Failed to fetch active listings. Please try again.', 'error');
    }
  }, [addToast]);
