...
const ChatView: React.FC<ChatViewProps> = ({
    ...
    useEffect(() => {
        ...
        setCurrentUserRole(role?.toUpperCase() || "SELLER");
        // Scroll to bottom when role changes
        scrollToBottom();
    }, [authLoading, role]);
            ...
            <div className="flex flex-wrap gap-1 mt-2">
                {Object.entries(reactionCounts).map(([emoji, count]) => {
                    const userReacted = message.reactions?.some(
                        r => r.emoji === emoji && r.reactorId === currentUserRole
                    );
    ...
    const renderMessageOptions = (message: ChatMessage) => {
        ...
        const canEdit = isFromCurrentUser && !message.deleted &&
            new Date().getTime() - new Date(message.sentAt).getTime() < 15 * 60 * 1000; // 15 minutes
        ...
        return (
            <div className="mb-2 p-2 border-l-2 border-gray-300 bg-gray-50 rounded">
                ...
                <p className="text-xs text-gray-600 truncate">
                    {message.replyTo.deleted ? 'This message was deleted' :
